...
from forecasting_v3 import forecast_ctr  # <-- NEW IMPORT
...
def run_pipeline_v2():
    ...
    logger.info("V3 pipeline started", extra={"run_id": run_id})
    ...
    try:
        ...
        # ---------- CTR Forecasting ------------------
        forecast_results = forecast_ctr(df_kpi, periods=7)
        forecast_path = os.path.join(CONFIG["output_dir"], "forecast_v3.json")

        with open(forecast_path, "w") as f:
            json.dump(forecast_results, f, indent=4)

        logger.info(
            "V3 pipeline fully completed",
